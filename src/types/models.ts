...
export interface ModelConfig {
  id: string; // UUID
  model_id: string; // The unique identifier used in API calls (no longer unique)
  name: string;
  type: ModelType;
  variant_name?: string; // Optional name to distinguish between different configurations of the same model_id
  configuration_id?: string; // Optional unique identifier for this specific configuration
  cost_per_mp: number;
  custom_cost: number;
  is_active: boolean;
  created_at: string;
...
export const PREDICTION_TYPE_OPTIONS = ["epsilon", "v_prediction"];
