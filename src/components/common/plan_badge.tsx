...
export function PlanBadge({
  ...
  const get_plan_styles = () => {
    switch (plan) {
      case "admin":
        return {
          badge: "badge-error dark:text-white",
      ...
      case "standard":
        return {
          badge: "badge-primary dark:text-white",
  ...
  if (variant === "badge") {
    return (
      <div className={`badge ${styles.badge} ${size_classes[size]} font-medium`}>
        ...
        {plan_info.name}
        {plan_info.isUnlimited && <span className="ml-1 text-xs">∞</span>}
      </div>
    );
  }
          ...
          <div className="mt-3">
            ...
            <ul className="text-sm space-y-1">
              ...
              {plan_info.features.length > 3 && (
                <li className="text-gray-500">+{plan_info.features.length - 3} more features</li>
              )}
            </ul>
          </div>
        )}
  ...
  return (
    <span className={`inline-flex items-center ${size_classes[size]} ${styles.text} font-medium`}>
      <span className="mr-1">{styles.icon}</span>
      {plan_info.name}
      {plan_info.isUnlimited && <span className="ml-1 text-xs opacity-75">∞</span>}
    </span>
  );
}
