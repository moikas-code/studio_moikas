...
export function useJobBasedImageGeneration() {
  ...
  const polling_interval = useRef<NodeJS.Timeout | null>(null);
        ...
        if (job) {
          ...
          if (job.status === "completed" || job.status === "failed") {
            ...
            if (job.status === "completed") {
              ...
              const is_moderation_error =
                error_message.includes("Content blocked:") ||
  ...
  const submit_job = useCallback(
    async (params: GenerationParams): Promise<JobGenerationResult | null> => {
      set_is_loading(true);
      ...
      try {
        ...
        const cleaned_params = {
          ...
          loras:
            params.loras?.filter((l) => l && l.path && typeof l.path === "string") || undefined,
          // Filter out invalid embeddings
          embeddings:
            params.embeddings?.filter((e) => e && e.path && typeof e.path === "string") ||
            undefined,
        };
        ...
        const is_moderation_error =
          message.includes("Content blocked:") || message.includes("CONTENT_MODERATION_VIOLATION");
