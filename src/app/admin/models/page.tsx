...
import type { ModelConfig, ModelFilters, ModelType } from "@/types/models";
import { MODEL_TYPE_OPTIONS, format_model_type, get_model_cost_in_mp } from "@/types/models";
...
function ModelManagementPageContent() {
  ...
  const delete_model = async (model_id: string, model_name: string) => {
    if (
      !confirm(`Are you sure you want to delete "${model_name}"? This action cannot be undone.`)
    ) {
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        toast.success(
          data.data?.deleted_model ? `Model "${data.data.deleted_model}" deleted` : "Model deleted"
        );
        fetch_models();
      } else {
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      <div className="mb-8">
        ...
        <p className="text-base-content/70">Configure AI models available in the platform</p>
      </div>
      ...
      <div className="card bg-base-100 shadow-xl mb-6">
        <div className="card-body">
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            {/* Filter Toggle */}
            <button onClick={() => set_show_filters(!show_filters)} className="btn btn-ghost gap-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <select
                    className="select select-bordered w-full"
                    value={
                      filters.supports_image_input === undefined
                        ? ""
                        : filters.supports_image_input.toString()
                    }
                    onChange={(e) =>
                      set_filters({
                        ...filters,
                    ...
                    <tr key={model.id}>
                      ...
                      <td>
                        <div className="min-w-0">
                          ...
                          <div className="text-xs text-base-content/50 flex items-center gap-1">
                            <code className="truncate max-w-[150px] sm:max-w-[200px]">
                              {model.model_id}
                            </code>
                            <button
                      ...
                      <td>
                        <div className="text-right">
                          <div className="font-mono">{get_model_cost_in_mp(model)} MP</div>
      ...
      {selected_model && (
        <div className="modal modal-open">
          <div className="modal-box max-w-3xl">
            <h3 className="font-bold text-lg mb-4">{selected_model.name} Details</h3>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div>Model ID:</div>
                  <div>
                    <code>{selected_model.model_id}</code>
                  </div>
                  {selected_model.variant_name && (
                    <>
                      <div>Variant:</div>
                      <div>
                  ...
                  <div>Cost:</div>
                  <div>
                    {get_model_cost_in_mp(selected_model)} MP (${selected_model.custom_cost})
                  </div>
                  <div>Status:</div>
                  <div>
                    <span
                      className={`badge ${selected_model.is_active ? "badge-success" : "badge-ghost"}`}
                    >
              ...
              <div>
                ...
                <div className="text-sm">
                  ...
                  {(selected_model.size_mode === "aspect_ratio" ||
                    selected_model.supports_both_size_modes) && (
                    <div>
                      Aspect Ratios: {selected_model.supported_aspect_ratios?.join(", ") || "None"}
                    </div>
                  )}
                  {(selected_model.size_mode === "pixel" ||
                    selected_model.supports_both_size_modes) && (
                    <div>
                      Pixel Sizes:
                      <ul className="ml-4">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    {selected_model.supports_cfg && (
                      <>
                        <div>CFG Scale:</div>
                        <div>
                          Default: {selected_model.default_cfg}, Max: {selected_model.max_cfg}
                        </div>
                      </>
                    )}
                    {selected_model.supports_steps && (
                      <>
                        ...
                        <div>
                          Default: {selected_model.default_steps}, Max: {selected_model.max_steps}
                        </div>
                      </>
                    )}
                  </div>
              ...
              {selected_model.type === "video" && (
                <div>
                  ...
                  <div className="text-sm">
                    <div>
                      Duration Options: {selected_model.duration_options?.join(", ") || "None"}{" "}
                      seconds
                    </div>
                    <div>
                      Audio Generation: {selected_model.supports_audio_generation ? "Yes" : "No"}
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="modal-action">
              <button className="btn" onClick={() => set_selected_model(null)}>
                Close
              </button>
            </div>
          </div>
