...
import { memu_workflow_schema, validate_request } from "@/lib/utils/api/validation";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const token_cost = Math.max(MIN_COST, Math.ceil(estimated_tokens / TOKENS_PER_MP));
    ...
    await save_message(supabase, session_data.id, result.response, "assistant", token_cost);
    ...
    return api_success({
      ...
      session_id: session_data.id,
      tokens_used: token_cost,
      remaining_tokens: subscription.renewable_tokens + subscription.permanent_tokens - token_cost,
    });
  } catch (error) {
...
async function get_workflow(supabase: SupabaseClient, workflow_id?: string) {
  ...
  const { data } = await supabase.from("memu_workflows").select("*").eq("id", workflow_id).single();
...
async function save_message(
  ...
  await supabase.from("memu_chat_messages").insert({
    session_id,
    content,
    role,
    tokens_used,
