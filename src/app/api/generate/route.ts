...
export async function POST(req: NextRequest) {
  try {
    ...
    if (
      (model_config.metadata?.enable_dynamic_pricing || validated.model === "fal-ai/lora") &&
      model_config.metadata?.cost_per_inference_second
    ) {
    ...
    try {
      ...
      if (is_any_lora) {
        ...
        if (validated.model_name) {
          ...
          console.log(
            "[Any LoRA] Using fal-ai/lora endpoint with custom model:",
      ...
      console.log("Fal.ai result structure:", {
        ...
        timings:
          "timings" in result && typeof result === "object"
            ? (result as { timings?: unknown }).timings
            : "no timings",
      });
      ...
      if (
        ...
        if (actual_total_cost > initial_total_cost && subscription.plan !== "admin") {
          ...
          const total_available =
            updated_subscription.renewable_tokens + updated_subscription.permanent_tokens;
          ...
          if (total_available < additional_cost) {
            ...
            throw new Error(
              `Insufficient tokens for actual generation cost. Need ${actual_total_cost} MP total, but only ${total_available + initial_total_cost} MP available`
            );
          }
      ...
      if (result && typeof result === "object") {
        ...
        if ("data" in result && result.data && typeof result.data === "object") {
          ...
          // Check for URL in data
            ...
            throw new Error(
              `Unexpected data structure. Keys found in data: ${Object.keys(data).join(", ")}`
            );
          }
        }
        ...
        // Other fallback options...
          ...
          throw new Error(
            `Unexpected result structure from image generation. Keys found: ${Object.keys(result).join(", ")}`
          );
        }
      } else {
      ...
      for (const image_url of final_images) {
        ...
        if (is_free_user && !token_result.is_admin) {
          const watermarked = await add_overlay_to_image_node(
      ...
      const error_message =
        generation_error instanceof Error ? generation_error.message : "Unknown error";
