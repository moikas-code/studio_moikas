...
export async function GET(req: NextRequest) {
  try {
    ...
    if (job.fal_request_id && job.status !== "completed" && job.status !== "failed") {
      try {
        console.log(
          `Checking fal.ai status for job ${job_id} with request ID: ${job.fal_request_id}`
        );
        ...
        if (status_response.status === "IN_PROGRESS") {
          ...
          if (
            status_response.metrics?.inference_time &&
            job.model &&
            job.metadata?.billing_type === "time_based"
          ) {
            const supabase = get_service_role_client();
          ...
          try {
                ...
                if (typeof img === "string") {
                  ...
                  // Check if there's a base64 field
                    ...
                    extracted_images.push(
                      `data:${img.content_type || "image/png"};base64,${img.base64}`
                    );
                  } else {
                    // Log the full structure for debugging
                    console.error("Image object structure:", JSON.stringify(img, null, 2));
                    console.error(
                      "Full result_response:",
                      JSON.stringify(result_response, null, 2)
                    );
            ...
            if (extracted_images.length > 0) {
              image_url =
                extracted_images.length === 1
                  ? extracted_images[0]
                  : JSON.stringify(extracted_images);
            }
        ...
        if (job.metadata) {
          update_data.metadata = {
            ...job.metadata,
            watermark_applied:
              image_url && job.metadata.is_free_user && job.metadata.plan !== "admin",
            num_images_generated:
              image_url && image_url.startsWith("[") ? JSON.parse(image_url).length : 1,
            // Store the accurate inference time from fal.ai metrics
            ...(status_response.metrics?.inference_time
              ? { inference_time: status_response.metrics.inference_time }
              : {}),
          };
        }
        ...
        if (update_error) {
          ...
          if (
            ...
            if (subscription?.plan !== "admin") {
              const final_cost =
                job.metadata?.final_cost_mp || job.metadata?.time_based_cost_mp || job.cost;
    ...
    if (current_status === "failed") {
      ...
      if (status_response.status === "COMPLETED") {
        ...
        if (
          result_response.images &&
          Array.isArray(result_response.images) &&
          result_response.images.length > 0
        ) {
        ...
        if (extracted_images.length > 0) {
          image_url =
            extracted_images.length === 1 ? extracted_images[0] : JSON.stringify(extracted_images);
        }
    ...
    return api_success({
      ...
      images: Array.isArray(images_to_return)
        ? images_to_return
        : images_to_return
          ? [images_to_return]
          : [], // Always return array
      ...
      created_at: job.created_at,
      completed_at:
        job.completed_at || (current_status === "completed" ? new Date().toISOString() : null),
      prompt: job.prompt,
      model: job.model,
