...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!user) {
      return handle_api_error(new Error("Unauthorized"));
    }
    ...
    if (validated.action === "reject" && !validated.rejection_reason) {
      return handle_api_error(new Error("Rejection reason is required"));
    }
    ...
    if (request_error || !request) {
      return handle_api_error(new Error("DMCA request not found"));
    }

    // Check if already processed
    if (request.status !== "pending" && request.status !== "reviewing") {
      return handle_api_error(new Error("DMCA request has already been processed"));
    }
