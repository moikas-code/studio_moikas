  ...
  try {
    ...
    if (file && typeof file !== "string" && "arrayBuffer" in file) {
      ...
      if (file.type === "text/plain") {
        ...
        return NextResponse.json(
          { error: "Only text and PDF files are supported for now." },
          { status: 400 }
        );
      }
      ...
      return NextResponse.json(
        { error: "Please upload a file or enter a topic/link." },
        { status: 400 }
      );
    }
    ...
    if (!token_result.success) {
      return NextResponse.json(
        { error: token_result.error || "Failed to deduct tokens" },
        { status: 500 }
      );
    }

    // Log the usage with proper metadata
    ...
    if (feature === "summary") {
      ...
      system_prompt =
        "Create an outline of the key points from the following text. Be direct and concise.";
    } else if (feature === "script") {
      system_prompt =
        "Write a detailed script based on the following topic or content. The script should be engaging, informative, and well-structured.";
    } else if (feature === "product_description") {
      system_prompt =
        "Write a compelling product description based on the following topic or content. Highlight key features, benefits, and use persuasive language.";
    } else if (feature === "video_description") {
      system_prompt =
        "Write a captivating video description based on the following topic or content. Make it suitable for YouTube or similar platforms, and include relevant keywords.";
    } else if (feature === "tweet") {
      system_prompt =
        "Generate a set of engaging tweets based on the following topic or content. Each tweet should be concise, interesting, and suitable for social media.";
    } else if (feature === "profile_bio") {
      system_prompt =
        "Write a creative and professional profile bio based on the following topic or content. Make it suitable for social media or professional platforms.";
    }
