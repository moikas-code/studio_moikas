...
export async function POST(req: NextRequest) {
  try {
    ...
    const { error: deletion_error } = await supabase
    ...
    if (deletion_error) {
      ...
      if (deletion_error.code === '23505') { // Unique violation
        return handle_api_error(
          new Error("Deletion request already exists for this account"),
    ...
    console.log(`Account deletion requested for user ${user.id}, scheduled for ${deletion_date.toISOString()}`)

    return api_success({
      message: "Account deletion request received",
      scheduled_deletion_date: deletion_date.toISOString(),
      grace_period_days: 30,
      instructions: "Your account will be deleted in 30 days. You can cancel this request by logging in before the deletion date."
    })
  } catch (error) {
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (deletion_error || !deletion_data) {
      return handle_api_error(
        new Error("No pending deletion request found"),
        "NO_DELETION_REQUEST",
        404
      )
    }
