...
import { require_auth, get_user_subscription, has_sufficient_tokens } from "@/lib/utils/api/auth";
import {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      if (!webhook_url) {
        console.warn(
          "No webhook URL configured. Video generation will use synchronous processing."
        );
        console.warn(
          "Set one of these environment variables: VERCEL_URL, NEXT_PUBLIC_APP_URL, NEXTAUTH_URL, or URL"
        );
      }
      ...
      const is_queue_result =
        "request_id" in fal_result || "id" in fal_result || "requestId" in fal_result;
      ...
      await supabase
        ...
        .update({
          status: "failed",
          error:
            generation_error instanceof Error
