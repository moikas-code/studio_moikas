 ...
 */
export async function fetch_active_models(type?: 'image' | 'video' | 'audio' | 'text'): Promise<ModelConfig[]> {
 ...
 */
export async function fetch_default_model(type: 'image' | 'video' | 'audio' | 'text'): Promise<ModelConfig | null> {
  const supabase = get_service_role_client();
...
/**
 * Fetch a specific model by model_id
 */
